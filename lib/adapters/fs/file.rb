...
class UFS::FS::File < UFS::FS
  # Retuns a UFS::FS::File (File Object).  The path, permissions, ownership & group can be 
  ...
  #
  # Examples:
  #   UFS::FS::File.new '/tmp/deleteme.txt'
  #   UFS::FS::File.new '/tmp/deleteme.txt', 755
  #   UFS::FS::File.new '/tmp/deleteme.txt', 755, 'joshaven'
  #   UFS::FS::File.new '/tmp/deleteme.txt', 755, 'joshaven', 'staff'
  #   UFS::FS::File.new '/tmp/deleteme.txt', nil, 'joshaven'     # The attributes are ordered, however they are ignored if nil.
  ...
  # Appends to a file or raises UFS::IOError.  This method does not do anything with new line characters.  If you
  # want to write a line of text see the :writeln method.  This method will railse UFS::IOError if fild doesn't exist
  # call :concat! to create the file by writing to it.
  #
  # Example:
  #   f = UFS::FS.touch '/tmp/deleteme'
  #   f << 'Hello'
  #   f.concat 'World'
  #   f.read                    #=> "HelloWorld"
  def concat(data)
...
[ 'exists?', 'create!', 'touch', 'read', 'size', 'bytes', 'destroy!', 'move', 'group', 'group!', 'group?', 'owner', 'owner!', 'owner?', 'permissions', 'permissions!', 'permissions?'].each do |meth|
  UFS::FS::File.add_class_method meth do |*args|
...
# long as that module is included prior to the instantation of the UFS::FS::File object.
...
::File.methods.each do |meth|
  ...
  when *["allocate", "pipe"]
    UFS::FS::File.add_class_method meth do
      ::File.send meth
    end
  ...
  when *["atime", "basename", "blockdev?", "catname", "chardev?", "compare", "copy", "ctime", "delete", "directory?", "dirname", 
    ...
    UFS::FS::File.add_instance_method meth do |*args|
